# Round 12 - EfficientNetB3 Training with MLflow
...
SPLIT_MANIFEST_PATH = os.path.join(
    PROJECT_ROOT,
    "round10_grouped_split_70_20_10.csv"
)
...
# Set this to True only when a new split is deliberately required.
...
MODEL_SAVE_PATH = os.path.join(
    BASE_DIR,
    "plastic_model_EfficientNetB3_round10.keras"
)

HISTORY_PATH = os.path.join(
    BASE_DIR,
    "training_history_EfficientNetB3_round10.csv"
)

METRICS_PATH = os.path.join(
    BASE_DIR,
    "evaluation_metrics_EfficientNetB3_round10.csv"
)
...
def load_dataset(root_dir):
    """
    Recursively load images from each plastic class.

    Both colour and grey image folders are included.
    Images with the same base filename are grouped together
    to reduce leakage between dataset splits.
    """
...
# Create or load the shared 70/20/10 split

def create_and_save_split():
    """
    Create the grouped stratified split.

    The first stage creates 70% training data and a
    temporary 30% set. The temporary set is then divided
    into 20% validation and 10% test data.
    """
    ...
    print("\nCreated Round 10 shared split file:")
    print(SPLIT_MANIFEST_PATH)


def load_split_from_csv():
    """
    Load the exact saved train, validation and test split.

    EfficientNetB3 and ResNet50 use the same split file.
    """
...
def process_image(path, label):
    """
    Read, decode, resize and convert each image to float32.

    EfficientNetB3 performs its required normalization
    internally.
    """
...
# A non-shuffled version is used for final training evaluation.
...
MODEL_SUMMARY_PATH = os.path.join(
    BASE_DIR,
    "model_summary_EfficientNetB3_round10.txt"
)
...
def evaluate_dataset(
    ...
    """
    Evaluate a dataset split and save its classification
    report, confusion matrix and prediction results.
    """
    ...
    report_path = os.path.join(
        BASE_DIR,
        f"classification_report_EfficientNetB3_round10_{split_name}.txt"
    )

    cm_path = os.path.join(
        BASE_DIR,
        f"confusion_matrix_EfficientNetB3_round10_{split_name}.csv"
    )

    prediction_path = os.path.join(
        BASE_DIR,
        f"prediction_results_EfficientNetB3_round10_{split_name}.csv"
    )
...
with mlflow.start_run(
    run_name="Round_10_EfficientNetB3_70_20_10"
):
    ...
    print("\n" + "=" * 70)
    print("TRAINING - ROUND 10 EFFICIENTNETB3")
    print("=" * 70)
    ...
    summary_json_path = os.path.join(
        BASE_DIR,
        "round10_summary_EfficientNetB3.json"
    )
    ...
    # Log generated files as MLflow artifacts.
    ...
    print("\n" + "=" * 70)
    print("ROUND 10 EFFICIENTNETB3 COMPLETED")
    print("=" * 70)
    ...
    print("\nMLflow experiment:")
    print("PlastiSort_Round_10_70_20_10")
