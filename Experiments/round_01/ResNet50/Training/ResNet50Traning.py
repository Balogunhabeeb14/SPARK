...
SPLIT_MANIFEST_PATH = os.path.join(
    PROJECT_ROOT,
    "round9_grouped_split_70_20_10.csv",
)
...
# Set to True only when a completely new split is required.
...
HISTORY_PATH = os.path.join(
    BASE_DIR,
    "training_history_ResNet50_round9.csv",
)

METRICS_PATH = os.path.join(
    BASE_DIR,
    "evaluation_metrics_ResNet50_round9.csv",
)
...
def load_dataset(root_dir):
    ...
    Colour and grey image folders are both included.
    Images that share the same base filename are grouped
    together to reduce data leakage between splits.
    """
...
def create_and_save_split():
    """
    Create a grouped stratified 70/20/10 split.

    The first split creates 70% training data and a
    ...
    print("\nCreated Round 9 shared split file:")
    print(SPLIT_MANIFEST_PATH)


def load_split_from_csv():
    """
    Load the previously saved train, validation and test split.
    """
...
def process_image(path, label):
    """
    Read, decode, resize and convert an image to float32.

    ResNet50 preprocessing is applied later by the
    preprocessing layer inside the model.
    """
...
# Separate training dataset without shuffling for final evaluation.
...
class MLflowMetricsCallback(tf.keras.callbacks.Callback):
    """
    Log training and validation metrics to MLflow after each epoch.
    """
...
class ResNet50Preprocess(layers.Layer):
    """
    Apply the standard ResNet50 preprocessing function
...
MODEL_SUMMARY_PATH = os.path.join(
    BASE_DIR,
    "model_summary_ResNet50_round9.txt",
)
...
def evaluate_dataset(
    ...
    report_path = os.path.join(
        BASE_DIR,
        f"classification_report_ResNet50_round9_{split_name}.txt",
    )

    cm_path = os.path.join(
        BASE_DIR,
        f"confusion_matrix_ResNet50_round9_{split_name}.csv",
    )

    prediction_path = os.path.join(
        BASE_DIR,
        f"prediction_results_ResNet50_round9_{split_name}.csv",
    )
...
with mlflow.start_run(
    run_name="Round_9_ResNet50_70_20_10"
):
    ...
    print("\n" + "=" * 70)
    print("TRAINING - ROUND 9 RESNET50")
    print("=" * 70)
    ...
    # Final evaluation on all three data splits.
    ...
    print("\n" + "=" * 70)
    print("ROUND 9 RESNET50 COMPLETED")
    print("=" * 70)
    ...
    print("\nMLflow experiment:")
    print("PlastiSort_Round_9_70_20_10")
